...
class Calligraph extends CalligraphBase
  ...
  subinit: ()->
    #hello!
    console.log 'Calligraph "Mosquitos" init'
  ...
  on_mousedown: (e) =>
    ...
    @attractionBehaviour.reset(@emitter.p, 8, 500)
    @n.saw1.start()
    @n.saw2.start()
    @n.saw3.start()
...
#     @haloContainer.position = {x: 0, y: 0}
    ...
    @render_recursive_stage = @create_recursive_render @feedback, 'capture', 'capture_alpha',  true, 1.0
    ...
    @crossZoneBehaviour = new Proton.CrossZone(new Proton.RectZone(0, 0, @canvas_width, @canvas_height), 'bound')
    $(window).on 'resize', ()=>
    ...
    @emitter.addBehaviour(new Proton.RandomDrift(20, 20, 0))
    ...
    @mosquitos = [] #stores the three main particles
    ...
    spark = PIXI.Texture.fromImage("images/spark_40x40_m.png")
    ...
    @proton_renderer.onParticleCreated = (particle,b,c) =>
      ...
      #make a sub-emitter
      if particle.parent.id is "emitter_0"
        @mosquitos.push particle
        sub = new Proton.BehaviourEmitter()
        sub.rate = new Proton.Rate(new Proton.Span(2,4), new Proton.Span(0.2,0.8))
  ...
  animate: (timeval) =>
    ...
    for moz,index in @mosquitos
      distance = Math.sqrt((moz.p.x - mousedata.lastX) * (moz.p.x - mousedata.lastX) + (moz.p.y - mousedata.lastY) * (moz.p.y - mousedata.lastY))
      @n["saw_track#{index+1}"]
      .param
        gain: 1 - Math.abs @clamp(distance*distance, 0, 10000, 0, 1.0)
        ramp:'expo'
        at: 0.01
      .param
        pan: @clamp(moz.p.x, 0, @canvas_width, -1.0, 1.0)
        ramp:'linear'
        at: 0.1
